...
const registerUser = async (data) => {
  try {
    ...
    await db.User.create({ ...data, phonenumber: data.phone, password: hashPassword, groupId: 4 });
    return {
      EM: "Register account new user successful",
      EC: 0,
      DT: [],
    };
  } catch (error) {
...
const getGroupShipper = async () => {
  try {
    ...
    if (users && users.length > 0) {
      return {
        EM: "Get group shipper success!",
        EC: 0,
        DT: users,
      };
    } else {
      return {
        EM: "Get group shipper error!",
        EC: 0,
        DT: [],
      };
    }
